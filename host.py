...
def self_ping():
    """
    Pings the local Flask server every 14 minutes to prevent Render free plan
    from sleeping. Render free plan sleeps after 15 minutes of inactivity,
    so we ping every 14 minutes (840 seconds) — no unnecessary busy-looping.
    For true 0-downtime, pair this with an external monitor like UptimeRobot
    pointing to your Render service URL.
    """
    while True:
        try:
            ...
            pass
        time.sleep(840)  # 14 minutes — just under Render's 15-min sleep threshold

# --- Configuration ---
TOKEN = '8598607651:AAE2mPszlOg9acRzXG6nvP0Wp5ShscsvKq4' # Replace with your actual token
                ...
                if return_code != 0 and stderr:
                    ...
                    if match_py:
                        ...
                        if attempt_install_pip(module_name, message_obj_for_reply):
                            ...
                            time.sleep(0)  # 0 sleep for Render free plan
...
def execute_broadcast(broadcast_text, photo_id, video_id, caption, admin_chat_id):
    ...
    for i, user_id_bc in enumerate(users_to_broadcast): # Renamed
        ...
        elif i % 5 == 0: time.sleep(0)  # 0 sleep for Render free plan
...
if __name__ == '__main__':
    ...
    keep_alive()  # Start Flask health endpoint
    threading.Thread(target=self_ping, daemon=True).start()  # Start keep-alive pinger
    ...
    while True:
        try:
            ...
            logger.warning("Polling ReadTimeout. Restarting immediately...")
        except requests.exceptions.ConnectionError as ce:
            logger.error(f"Polling ConnectionError: {ce}. Restarting immediately...")
        except Exception as e:
            logger.critical(f"💥 Unrecoverable polling error: {e}", exc_info=True)
            logger.info("Restarting polling immediately...")
        finally:
